...
<body class="content1">
	...
	<div class="container-fluid">
		<div class="row-fluid">
			<form method="post" action="RoleServlet?obj=updaterolefun&roleid=${roleid }">
				...
				<div class="well">
					<c:forEach items="${list}" var="fun">
						<c:if test="${fun.getFunpid() == 0}">
							<div class="toc">
								<h3>
									<input type="checkbox" onclick="selectparent(this)"  name="ckrr" value="${fun.getFunid() }" <c:if test="${fun.isRr() == true}">checked="checked"</c:if>  />
									${fun.getFunname() }
								</h3>
								<ul>
									<c:forEach items="${list}" var="child">
										<c:if test="${child.getFunpid() == fun.getFunid() }">								
											<li>
												<input type="checkbox" onclick="selectchild(this)"  name="ckrr" value="${child.getFunid() }" <c:if test="${child.isRr() == true}">checked="checked"</c:if> />
												${child.getFunname() }
											</li>
										</c:if>
									</c:forEach>
