...
<body style="background-color:#f2f9fd;">
    ...
    <div class="leftnav">    		
   		<div class="leftnav-title"><strong><span class="icon-list"></span>菜单列表</strong></div>
   			<c:forEach items="${rolefun}" var="fun">
   				<c:if test="${fun.getFunpid() == 0 && fun.isRr() == true}">
			        <h2><span class="icon-briefcase"></span>${fun.getFunname() }</h2>
			        <ul id="error-menu" class="nav nav-list collapse" style="display:block">
			        	<c:forEach items="${rolefun}" var="child">
			        		 <c:if test="${child.getFunpid() == fun.getFunid() && child.isRr() == true}">
					             <li>
					                 <a href="${child.getFunurl() }" target="right"><span class="icon-caret-right"></span>${child.getFunname() }</a>
					             </li>
				             </c:if>
			             </c:forEach>
			        </ul>
